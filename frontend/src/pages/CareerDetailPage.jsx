...
export default function CareerDetailPage() {
  ...
  return (
    <>
      <div className="hero">
        <Link to="/careers" className="small" style={{ color: "var(--accent)" }}>← All careers</Link>
              ...
              {(advice && !adviceErr && (advice.next_steps || []).length)
                ? advice.next_steps.map((s, i) => (
                    <div key={i} className="step-card" style={{ border: "1px solid var(--border)", background: "var(--panel-2)", borderRadius: 14, padding: 16 }}>
                      ...
                      <button className="btn-ghost mt" onClick={() => { if (s.link) navigate(s.link.startsWith("/") ? s.link : `/${s.link}`); }}>Take this step →</button>
                    </div>
                  ))
