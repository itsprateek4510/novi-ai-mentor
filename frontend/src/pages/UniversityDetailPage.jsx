...
export default function UniversityDetailPage() {
  ...
  return (
    <>
      <div className="hero"><Link to="/universities" className="small" style={{ color: "var(--accent)" }}>← All universities</Link><h1 className="mt">{esc(u.name)}</h1><p>{esc(u.city)}, {esc(u.country)} · <b>{esc(u.course)}</b> · Rank #{u.ranking || "—"}{(u.courses || []).length ? ` · ${u.courses.length} courses` : ""}</p></div>
      <div className="cols">
